...
export class StatsComponent implements OnInit {
  ...
  initializeCharts() {
    let bestScores: Array<Array<string>> = new Array<Array<string>>();
    let finalBestScores = [];
    let currentRankingsPoints: Array<Array<string>> = new Array<Array<string>>();
    ...
    this.dataService.getUserPeriodBestScores(this.user.id, this.from, this.to).subscribe(
      next => {
        // for (let i of next){
        //   bestScores[i] = next[i].split;
        // }
        ...
        for (let i = 0; i < next.length; i++) {
          console.log(bestScores[i] = next[i].split(','));
        }
        for (let i = 0; i < bestScores.length; i++) {
          finalBestScores[i] = ({
            x: new Date(bestScores[i][0]),
            y: +bestScores[i][1]
          });
          console.log(finalBestScores[i].y);
        }
    ...
    this.dataService.getUserPeriodCurrentRankingsPoints(this.user.id, this.from, this.to).subscribe(
      next => {
        console.log('first' + next);
        if (next.length === 0) {
          this.shortage[1] = 'There is not any data to rankings Points chart';
        }
        for (let i = 0; i < next.length; i++) {
          console.log(currentRankingsPoints[i] = next[i].split(','));
        }

        for (let i = 0; i < currentRankingsPoints.length; i++) {
          if (i === 0) {
            difference = +currentRankingsPoints[i][1];
          } else {
            difference = +currentRankingsPoints[i][1] - (+currentRankingsPoints[i - 1][1]);
          }
          if (difference > 0) {
            indexLabel = `+${difference}`;
            markerColor = 'green';
          ...
          finalCurrentRankingsPoints[i] = ({
            x: new Date(currentRankingsPoints[i][0]),
            y: +currentRankingsPoints[i][1],
            indexLabel: indexLabel, markerColor: markerColor, markerType: markerType
          });
          console.log(finalCurrentRankingsPoints[i].y);
        }
        const chart2 = new CanvasJS.Chart('chartContainer2', {
          ...
          data: [{
            ...
            type: 'line',
            dataPoints: finalCurrentRankingsPoints
          }]
        });
    ...
    this.dataService.getPeriodGamers(this.from, this.to).subscribe(
      next => {
        // for (let i of next){
        //   bestScores[i] = next[i].split;
        // }
        ...
        for (let i = 0; i < currentGamers.length; i++) {
          finalCurrentGamers[i] = ({
            x: new Date(currentGamers[i][0]),
            y: +currentGamers[i][1]
          });
          console.log('pair: ' + finalCurrentGamers[i]);
        }
  ...
  searchInterval() {
    this.from = formatDate(this.from, 'yyyy-MM-dd', 'en-UK');
    this.to = formatDate(this.to, 'yyyy-MM-dd', 'en-UK');
    this.initializeCharts();
  }
